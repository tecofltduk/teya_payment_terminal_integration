...
class PosPaymentMethod(models.Model):
    ...
    def nf_get_teya_stores(self):
        ...
        for store in stores:
            ...
            terminal_response = requests.request("GET", terminals_url, headers=headers, data=payload, timeout=30)
            if terminal_response.status_code == 200:
                terminals = terminal_response.json().get("terminals") or []
                for terminal in terminals:
                    find_store = self.env["teya.terminal.device"].search(
    ...
    def teya_payment_request_status(self, payment_req_id, pos_session_id):
        if not payment_req_id or str(payment_req_id).lower() in ("none", "false", "0"):
            return {
                "status_code": 400,
                "message": _("Missing payment request id; start a new payment after fixing the Teya error."),
            }
